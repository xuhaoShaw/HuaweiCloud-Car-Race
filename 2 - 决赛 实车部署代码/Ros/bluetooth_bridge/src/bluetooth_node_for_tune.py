...
from bluetooth_bridge.msg import BluetoothCtrMsg, LaneMsg
import struct
import threading
import os

BASE_DIR = os.path.abspath(os.path.dirname(__file__))
SAVE_FILE = BASE_DIR + '/data.npy'
...
class Application:
    ...
    topic_lane_det = "/lane_detect"    # 车道线数据
    ...
    def __init__(self):
        ...
        self.sub_lane_det   = rospy.Subscriber(self.topic_lane_det, LaneMsg, self.save_data_callback)
                ...
                while self.is_running: #--- receive data
                    ...
                    if ready[0]:
                        ...
                        if self.check_crc(data): # pub control msg
                            ...
                            decode_msg = self.decode_bt_data(data)
                            # 保存数据
                            self.ctr_data[0] = decode_msg.direction
                            self.ctr_data[1] = decode_msg.speed
                            self.ctr_data[2] = decode_msg.mode
                            self.pub_bt_decode.publish(decode_msg)
                            if (ord(data[5])):
    ...
    def sigint_handler(self, signal, frame):
        ...
        print TAG,"Interrupt!"
        self.data_record = np.vstack(self.data_record)
        print TAG, "Save Data!"
        np.save(SAVE_FILE, self.data_record)
