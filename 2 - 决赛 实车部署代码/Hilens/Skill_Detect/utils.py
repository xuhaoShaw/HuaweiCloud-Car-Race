...
# 检测模型输入尺寸，需要将此处字符串替换为真实的输入宽高，比如 (352, 640)
(net_h, net_w) = int(image_shape[0]), int(image_shape[1])

# 检测模型的类别，需要将此处字符串换为真实的标签列表，比如 ['car', 'person', 'light']
...
def draw_boxes(img_data, bboxes):
    ...
    for bbox in bboxes:
        ...
        cv2.putText(img_data, class_names[label], (x_min, y_min - 20), text_font, font_scale, (255, 255, 0), thickness)
