...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in [-1, 0]:
        ...
        logger_tb = SummaryWriter(str(save_dir))
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            if plots:
                ...
                if logger_tb:
                    logger_tb.add_histogram('classes', c, 0)  # TensorBoard
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    if logger_tb and ni == 0:  # TensorBoard
                        with warnings.catch_warnings():
                            warnings.simplefilter('ignore')  # suppress jit trace warning
                            logger_tb.add_graph(torch.jit.trace(de_parallel(model), imgs[0:1], strict=False), [])
            # end batch ------------------------------------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                if logger_tb:
                    logger_tb.add_scalar(tag, x, epoch)  # TensorBoard
