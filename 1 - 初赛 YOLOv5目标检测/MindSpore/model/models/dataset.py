...
from transforms import reshape_fn, MultiScaleTrans, PreprocessTrueBox, DistributedSampler
...
def create_yolo_dataset(image_dir, anno_path, batch_size, max_epoch=500,
    ...
    yolo_dataset = COCOYoloDataset(root=image_dir, ann_file=anno_path, filter_crowd_anno=filter_crowd, multi_proc=False,
                                   remove_images_without_annotations=remove_empty_anno, is_training=is_training)
    hwc_to_chw = CV.HWC2CHW()

    num_workers = multiprocessing.cpu_count()
    if is_training:
        multi_scale_trans = MultiScaleTrans(config)
        ...
        multi_proc = False
        ds = de.GeneratorDataset(yolo_dataset, column_names=dataset_column_names, shuffle=shuffle,
                                 python_multiprocessing=multi_proc, num_parallel_workers=min(4, num_workers))
        ds = ds.map(operations=multi_scale_trans, input_columns=dataset_column_names,
                    ...
                    num_parallel_workers=min(12, num_workers), python_multiprocessing=multi_proc)
        ds = ds.map(operations=PreprocessTrueBox(config), input_columns=map2_in_column_names,
                    ...
                    num_parallel_workers=min(4, num_workers), python_multiprocessing=multi_proc)
        mean = [m * 255 for m in [0.485, 0.456, 0.406]]
        std = [s * 255 for s in [0.229, 0.224, 0.225]]
        ds = ds.map([CV.Normalize(mean, std),
                     hwc_to_chw], num_parallel_workers=min(4, num_workers))

        def concatenate(images):
            images = np.concatenate((images[..., ::2, ::2], images[..., 1::2, ::2],
                                     images[..., ::2, 1::2], images[..., 1::2, 1::2]), axis=0)
            return images
        ds = ds.map(operations=concatenate, input_columns="image", num_parallel_workers=min(4, num_workers))
        ds = ds.batch(batch_size, num_parallel_workers=min(4, num_workers), drop_remainder=True)
    ...
    data_loader = ds.create_dict_iterator(output_numpy=True, num_epochs=max_epoch)  # 最多迭代500次
